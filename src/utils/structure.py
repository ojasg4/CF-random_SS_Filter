...
def extract_models_from_pse(
    pse_path: Path, extracted_pdb_dir: Path, raw_dssp_dir: Path
) -> List[ExtractedModel]:
    ...
    with pymol_session() as cmd:
        ...
        for obj in objects:
            ...
            if atom_count == 0:
                continue
            pdb_path = extracted_pdb_dir / f"{obj}.pdb"
            dssp_path = raw_dssp_dir / f"{obj}.dssp"
    ...
    if not models:
        raise ValueError(
            f"No molecular PDB-like objects were extracted from {pse_path}"
        )
    return models
...
def extracted_models_from_keep_going_pdb_files(
    ...
    for pdb_path in pdb_files:
        model_dir_name = pdb_path.parent.name
        model_name = relative_path_for_summary(pdb_path, pse_parent)
        models.append(
            ExtractedModel(
                model_name=model_name,
                pdb_path=pdb_path,
                dssp_path=raw_dssp_dir / model_dir_name / f"{pdb_path.stem}.dssp",
            )
        )
    return models
...
def analyze_alternatives_with_dssp(
    ...
    rmsd_results = calculate_rmsds_to_dominant(dominant_model, alternative_models)
